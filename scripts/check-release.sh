...
: > "$TMP_TITLES"
for d in "${DOMAINS_CONTENT[@]}"; do
    # --compressed на случай gzip от nginx; HTML кладём в одну строку
    html=$(curl_d "$d" -s --compressed --max-time 10 "https://$d/" 2>/dev/null | tr -d '\n\r')
    if [[ -z "$html" ]]; then
        bad "$d — пустой HTML (сайт не раздаётся)"
        continue
    fi
    # title — простой <title>plain</title>
    t=$(echo "$html" | sed -n 's|.*<title[^>]*>\([^<]*\)</title>.*|\1|p' | head -1)
    # h1 в Hero обёрнут в <span> детьми, поэтому жадно ловим <h1...>...</h1>,
    # снимаем теги внутри и схлопываем пробелы. macOS bash 3.2 / sed BRE.
    h1=$(echo "$html" \
        | sed -n 's|.*<h1[^>]*>\(.*\)</h1>.*|\1|p' \
        | head -1 \
        | sed 's|<[^>]*>||g' \
        | sed 's/  */ /g; s/^ *//; s/ *$//')
    if [[ -z "$t" && -z "$h1" ]]; then
        bad "$d — не удалось извлечь ни title, ни h1 (сайт пустой?)"
    elif [[ -z "$t" ]]; then
        bad "$d — нет <title> в HTML"
    elif [[ -z "$h1" ]]; then
        bad "$d — нет <h1> в HTML"
    else
        inf "$d → title: $(echo "$t" | cut -c1-60)…"
        inf "$d → h1:    $(echo "$h1" | cut -c1-60)…"
        printf "%s\t%s\n" "$t" "$d" >> "$TMP_TITLES"
    fi
done
