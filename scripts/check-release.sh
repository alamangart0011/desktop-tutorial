...
for d in "${DOMAINS_CONTENT[@]}"; do
    code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 -H "User-Agent: check-release/1.0" "https://$d/" 2>/dev/null)
    case "$code" in
        200) ok "https://$d/ → HTTP 200" ;;
        301|302) yel "https://$d/ → HTTP $code (редирект — норма для www, но на корне странно)" ;;
        000) bad "https://$d/ — не отвечает (порт 443 закрыт? Нет certbot-сертификата?)" ;;
...
for d in "${DOMAINS_CONTENT[@]}"; do
    exp=$(echo | openssl s_client -servername "$d" -connect "$d:443" -verify_return_error 2>/dev/null \
...
for d in "${DOMAINS_CONTENT[@]}"; do
    robots=$(curl -s --max-time 10 "https://$d/robots.txt" 2>/dev/null)
    ...
    sm_code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 "https://$d/sitemap.xml" 2>/dev/null)
    if [[ "$sm_code" == "200" ]]; then
        sm=$(curl -s --max-time 10 "https://$d/sitemap.xml" 2>/dev/null | grep -oE "<loc>[^<]+" | head -1)
...
for d in gisprof.ru profilaktika-spb.ru; do
    code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 "https://$d/privacy" 2>/dev/null)
