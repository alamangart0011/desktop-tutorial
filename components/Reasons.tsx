      ...
      <div className="container-x">
        ...
        <div className="mt-10 grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {ITEMS.map(({ Icon, t, d }) => (
            <div
              key={t}
              className="rounded-2xl border border-slate-200 bg-[var(--color-paper)] p-6 hover:border-[var(--color-brand-2)]/60 hover:shadow-lg transition"
            >
              <div className="w-12 h-12 rounded-xl bg-[var(--color-brand)]/10 text-[var(--color-brand)] flex items-center justify-center">
                <Icon className="w-6 h-6" />
              </div>
              <div className="mt-4 font-bold text-[var(--color-ink)] text-lg leading-snug">
                {t}
              </div>
